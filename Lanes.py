...
class LaneLine:
    ...
    def updateLine(self, new_fit_pixel, new_fit_meter, detected, clear_buffer=False):
        """
    ...
    def drawLine(self, mask, color=(255, 0, 0), line_width=50, average=False):
        """
...
def getFitsBySlidingWindows(birdeye_binary, line_lt, line_rt, n_windows=9, verbose=True):
    """
    ...
    line_lt.updateLine(left_fit_pixel, left_fit_meter, detected=detected)
    line_rt.updateLine(right_fit_pixel, right_fit_meter, detected=detected)
...
def getFitsByPreviousFits(birdeye_binary, line_lt, line_rt, verbose=True):
    """
    Get polynomial coefficients for lane-lines detected in an binary image.
...
def drawBackOntoRoad(img_undistorted, Minv, line_lt, line_rt, keep_state):
    """
    ...
    line_warp = line_lt.drawLine(line_warp, color=(255, 0, 0), average=keep_state)
    line_warp = line_rt.drawLine(line_warp, color=(0, 0, 255), average=keep_state)
...
if __name__ == '__main__':
    ...
    line_lt, line_rt = LaneLine(buffer_len=10), LaneLine(buffer_len=10)
    ...
    for test_img in glob.glob('test_images/*.jpg'):
        ...
        binary_Images,img_binary= binaryImage(img_undistorted)

        img_birdeye, M, Minv,birdEye_Image = birdeye(img_binary)

        line_lt, line_rt, img_out = getFitsBySlidingWindows(img_birdeye, line_lt, line_rt, n_windows=7, verbose=True)
