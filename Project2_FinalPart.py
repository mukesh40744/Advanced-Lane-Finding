...
processed_frames = 0  
line_lt =LaneLine(10)  # line on the left of the lane
line_rt =LaneLine(10)  # line on the right of the lane


def showFinalFrame(blend_on_road, img_binary, img_birdeye, img_fit, line_lt, line_rt, offset_meter):
    """
...
def calcOffsetFromCenter(line_lt, line_rt, frame_width):
    """
...
def laneDetection(frame,mtx,dist, keep_state=True):
    """
    ...
    if processed_frames > 0 and keep_state and line_lt.detected and line_rt.detected:
        line_lt, line_rt, img_fit =getFitsByPreviousFits(img_birdeye, line_lt, line_rt, verbose=False)
    else:
        line_lt, line_rt, img_fit =getFitsBySlidingWindows(img_birdeye, line_lt, line_rt, n_windows=9, verbose=False)

    # compute offset in meter from center of the lane
    offset_meter =calcOffsetFromCenter(line_lt, line_rt, frame_width=frame.shape[1])

    # draw the surface enclosed by lane lines back onto the original frame
    blend_on_road =drawBackOntoRoad(img_undistorted, Minv, line_lt, line_rt, keep_state)

    # stitch on the top of final output images from different steps of the pipeline
    blend_output = showFinalFrame(blend_on_road, img_binary, img_birdeye, img_fit, line_lt, line_rt, offset_meter)
